...
get_item_contents <- function(desc) {
  ...
  contents <- list(
    is_beer = FALSE,
    contents = tibble(n = integer(), beer = character(), price = numeric())
  )
  ...
  non_beer_terms <- c("chemex", "opener", "sinus", "cold brew", "whole bean")
...
jj %>%
  ...
  write.csv(paste0("data/", Sys.Date(),'_thb','.csv'))
